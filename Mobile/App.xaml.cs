    ...
    sealed partial class App : Application
    {
        ...
        private async void AppUnhandledException(object sender, Windows.UI.Xaml.UnhandledExceptionEventArgs e)
        {
            ...
            if (e.Exception is FaultException)
            {
                ...
                async () =>
                {
                    var mensajeDialogo = new MessageDialog(e.Message, "MOBILE");
                    await mensajeDialogo.ShowAsync();
                });
            }
            else
                CrearErrorCliente(e.Message, e.Exception);
            UnhandledException -= AppUnhandledException2;
            UnhandledException += AppUnhandledException;
        }
        ...
        private async void AppUnhandledException3(object sender, Windows.UI.Xaml.UnhandledExceptionEventArgs e)
        {
            ...
            async () =>
            {
                var mensajeDialogo = new MessageDialog($"Ha ocurrido un inconveniente y no se pudo loguear.{Environment.NewLine}Mensaje : {mensaje}", "MOBILE");
                await mensajeDialogo.ShowAsync();
            });
        }

        private async void CrearErrorCliente(string mensaje, Exception exception)
        {
            ...
            var endpoint = rmap.GetValue("DireccionServicioMobile", ctx).ValueAsString;
            var servicio = new ServicioMobileClient(binding, new EndpointAddress(endpoint));
            ...
            async () =>
            {
                var mensajeDialogo = new MessageDialog($"Ha ocurrido un inconveniente.{Environment.NewLine}Repórtelo con el código : {id}", "MOBILE");
                await mensajeDialogo.ShowAsync();
            });
        }
        ...
        private async void LoguearError(Exception ex)
        {
            ...
            async () =>
            {
                var mensajeDialogo = new MessageDialog($"Ha ocurrido un inconveniente.{Environment.NewLine}Comuniquese con IT.", "MOBILE");
                await mensajeDialogo.ShowAsync();
            });
        }

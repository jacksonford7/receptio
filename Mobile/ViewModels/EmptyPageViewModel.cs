...
{
    internal class EmptyPageViewModel : NotificationBase
    {
        #region Variables
        private readonly EmptyPage _ventanaAutenticacion;
        ...
        private ObservableCollection<ListItem> _liEDO;
        private string Iso;
        ...
        private bool flat1 = false;
        private bool habilita = false;
        private string tam = "";

        private ListItem _cargaimp1;
        private ListItem _cargaimp2;
        private ListItem _cargaimp3;
        private ListItem _cargaimp4;
        private ListItem _cargaDepo;
        ...
        private ObservableCollection<ListItem> _lsDepositos;
        ...
        #region Constructor
        internal EmptyPageViewModel(EmptyPage ventanaAutenticacion)
        {
            ...
            lsEDO = new ObservableCollection<edo_control_item>();
            liEDO = new ObservableCollection<ListItem>();
            liDeposito = new ObservableCollection<ListItem>();
        ...
        public ListItem CargaDepo
        {
            ...
            set
            {
                if (_cargaDepo == value)
                    return;
                _cargaDepo = value;
                RaisePropertyChanged("CargaDepo");
            }
        }
        ...
        public ObservableCollection<edo_control_item> lsEDO
        {
            get
            {
                return _lsEDO;
        ...
        public ObservableCollection<ListItem> liEDO
        {
            get
            {
                return _liEDO;
            ...
            set
            {
                SetProperty(ref _liEDO, value);
            }
        }

        public ObservableCollection<ListItem> liDeposito
        {
            get
            {
                return _lsDepositos;
            ...
            set
            {
                SetProperty(ref _lsDepositos, value);
            }
        }
        #endregion
        ...
        {
            return !(CargaImp1==null) || !string.IsNullOrWhiteSpace(CargaExp1);
        }
        ...
        public async void Ingresar()
        {
            ...
            else
            {
                if (!val && (farb1 == 1 || farb2 == 1))
                {
                    ...
                    if (farb1 == 1 || farb2 == 1)
                    {
                        var mensajeDialogo = new MessageDialog("Problema", "No se pueden procesar unidades del servicio SAV");
                        await mensajeDialogo.ShowAsync();
                        bvalida = true;
                        return;
                    }
            ...
            if (CargaExp1 != null && CargaExp1.Trim().Length > 0)
            {
                try
                {
                    ...
                    if (CargaImp1 != null && CargaImp1.Dato.Trim().Length > 0)
                    {
                        ...
                        if (tipfcl)
                        {
                            var mensajeDialogo = new MessageDialog("Problema", "No se puede procesar unidad FULL si no hay transacción MIXTA");
                            await mensajeDialogo.ShowAsync();
                            bvalida = true;
                            return;
                        }
            ...
            else
            {
                if (CargaImp1 != null && CargaImp1.Dato.Trim().Length > 0)
                {
                    ...
                    {
                        LineaEdo = CargaImp1.Dato.Substring(4, 3);
                        tam = CargaImp1.Dato.Substring(8, 1);

                        if (CargaImp1.Dato.Substring(13, 2) == "PF" || CargaImp1.Dato.Substring(12, 2) == "PL" || CargaImp1.Dato.Substring(12, 2) == "PC" || CargaImp1.Dato.Substring(12, 2) =="PS")
                            flat1 = true;
                        ...
                        if (CargaImp2 != null && CargaImp2.Dato.Trim().Length > 0)
                        {
                            ...
                            if ((CargaImp2.Dato.Substring(13, 2) == "PF" || CargaImp2.Dato.Substring(12, 2) == "PL" || CargaImp2.Dato.Substring(12, 2) == "PC" || CargaImp2.Dato.Substring(12, 2) == "PS") && flat)
                                flat = true;
                            else
                                flat = false;

                            if ((tam != "2" || CargaImp2.Dato.Substring(8, 1)!="2") && (!flat || !flat1))
                            {
                                val = false;
                        ...
                        if (CargaImp3 != null && CargaImp3.Dato.Trim().Length > 0)
                        {
                            ...
                            if ((CargaImp3.Dato.Substring(13, 2) == "PF" || CargaImp3.Dato.Substring(12, 2) == "PL" || CargaImp3.Dato.Substring(12, 2) == "PC" || CargaImp3.Dato.Substring(12, 2) == "PS") && flat)
                                flat = true;
                            else
                                flat = false;
                            ...
                            {
                                val = false;
                                mns = "No puede solicitar más de 2 unidades si no son de 20/plataformas";
                                LlenarGrid("Problema", "ERROR", mns, new SolidColorBrush(Colors.Red));
                                bvalida = true;
                                return;
                            }
                        ...
                        if (CargaImp4 != null && CargaImp4.Dato.Trim().Length > 0)
                        {
                            ...
                            if ((CargaImp4.Dato.Substring(13, 2) == "PF" || CargaImp4.Dato.Substring(12, 2) == "PL" || CargaImp4.Dato.Substring(12, 2) == "PC" || CargaImp4.Dato.Substring(12, 2) == "PS") && flat)
                                flat = true;
                            else
                                flat = false;
                            ...
                            {
                                val = false;
                                mns = "No puede solicitar más de 3 unidades si no son plataformas";
                                LlenarGrid("Problema", "ERROR", mns, new SolidColorBrush(Colors.Red));
                                bvalida = true;
                                return;
                            }
                        ...
                        int v_stock = -1;
                        string[] v_cadena = CargaImp1.Dato.Split('-');
                        int v_LineaPermitida = await s.UseStockControlAsync(v_cadena[0]); //verifica si la linea maneja inventario

                        if (v_LineaPermitida > 0)
                        {
                            if (CargaDepo == null)
                            {
                                LlenarGrid("Error", "Datos", "Para la línea seleccionada se debe escoger un depósito", new SolidColorBrush(Colors.Red));
                                bvalida = true;
                                return;
                            }

                            v_stock = await s.Get_Actual_StockAsync(int.Parse(CargaDepo.Dato.ToString().Trim()), v_cadena[0]);

                            if (v_stock <= 0)
                            {
                                LlenarGrid("Problema", "Transacción Tablet", "Depósito seleccionado se quedó sin stock. ", new SolidColorBrush(Colors.Red));
                                bvalida = true;
                                return;
                            }

                            int v_qty = 0;
                            if (CargaImp1 != null && CargaImp1.Dato.Trim().Length > 0) { v_qty += 1; }
                            if (CargaImp2 != null && CargaImp2.Dato.Trim().Length > 0) { v_qty += 1; }
                            if (CargaImp3 != null && CargaImp3.Dato.Trim().Length > 0) { v_qty += 1; }
                            if (CargaImp4 != null && CargaImp4.Dato.Trim().Length > 0) { v_qty += 1; }

                            if (v_stock < v_qty)
                            {
                                LlenarGrid("Problema", "Transacción Tablet", "Depósito seleccionado no tiene stock suficiente. ", new SolidColorBrush(Colors.Red));
                                bvalida = true;
                                return;
                            }
                        }
                        ...
                        if(v_LineaPermitida > 0)
                            {
                            foreach (pre_gate_details_item v_fila in cabt.pre_gate_details)
                            {
                                if (v_fila.reference_id == "I")
                                {
                                    var v_save = new save_stock_transactionRequest();
                                    v_save.depo_id = int.Parse(CargaDepo.Dato.ToString().Trim());
                                    v_save.line_op = v_cadena[0];
                                    v_save.notes = "RECEPTIO MOBILE - BAJA DE INVENTARIO (-)";
                                    v_save.qty = 1;
                                    
                                    v_save.pgate = long.Parse(trMa.OnError);
                                    v_save.objt = Host;
                                    v_save.pgdetid = v_fila.id;
                                    v_save.user = Usuario;
                                    await s.save_stock_transactionAsync(v_save);
                                }
                            }
                        }
        ...
        private async Task ValidarAisv(string pase, int num)
        {
                ...
                else
                {
                    ...
                    else
                    {
                        if (fcea.msg != null)
                            ...
                            mns= "No se puede procesar UNIDAD";
            ...
            if (aisv != null)
            {
                ...
                {
                    mns = "La carga ingresada no es del tipo CONTENEDOR";
                    val = false;
                    LlenarGrid(TransExp, "Error AISV/Cont", mns, new SolidColorBrush(Colors.Red));
                    var mensajeDialogo = new MessageDialog("Problema", mns);
                    await mensajeDialogo.ShowAsync();
                    return;
                }
        ...
        private async Task ValidarEdo(string pase, int num)
        {
            ...
            //var pr = new edo_item();
            //pr.tipe = pase.Substring(0,3);
            var ps = new edo_data_validationRequest();
            ps.edo_id = pase.Substring(0, 3);
            ps.chofer = chofer;
            ps.placa = placa;
            var pt = await s.edo_data_validationAsync(ps);

            if (!pt.edo_data_validationResult)
            {
                ...
                val = false;
                LlenarGrid(TransImp, "Error EDO", pt.msg != null ? pt.msg : "EDO NO AUTORIZADO", new SolidColorBrush(Colors.Red));
                
            }
            else
            {
                LlenarGrid(TransImp, "EDO", "Validación OK", new SolidColorBrush(Colors.Green));
                val = true;
            }
        ...
        private async Task ValidaRepImp(string pase, int num)
        {
            if (CargaImp2 != null)
                if (CargaImp1.Dato == CargaImp2.Dato)
                {
                    //valida chofer
                    if (val)
                        mns = "Verifique que no esté ingresando registros repetidos";
                    val = false;
                    LlenarGrid(TransImp, "Error EDO", mns, new SolidColorBrush(Colors.Red));

                }
        }

        private async Task ValidaRepExp(string pase, int num)
        {
            if (CargaExp2 != null && CargaExp2.Trim().Length > 0)
                if  (CargaExp1.Trim() == CargaExp2.Trim())
                {
                    //valida chofer
                    if (val)
                        mns = "Verifique que no esté ingresando registros repetidos";
        ...
        private void LlenaDetalleM(string carga, int tip, string dae, string refe)
        {
            ...
            //llenas detalle de transact
            dett.status = "N";
            dett.transaction_number = (refe == "I" && CargaDepo != null ? carga + "-" + CargaDepo.Dato : carga);
            dett.transaction_type_id = tip;
            if (dae != null && dae.Trim().Length > 0)
                dett.document_id = dae.Trim();
        ...
        private async void ListEDO()
        {
            lsEDO = await s.get_edo_control_listAsync();
            ListItem c;

            for (int i = 0; i < lsEDO.Count; i++)
            {
                c = new ListItem();
                c.Dato = lsEDO[i].id + "-" + lsEDO[i].line + "-" + lsEDO[i].iso + "-" + lsEDO[i].group + "-" + lsEDO[i].qty;
                c.Info = lsEDO[i].id + "-" + lsEDO[i].line + "-" + lsEDO[i].iso;
                liEDO.Add(c);
            }
        ...
        private async void ListDeposito()
        {
            if (CargaImp1 == null) return;
            string[] v_cadena = CargaImp1.Dato.Split('-');
            var ListDepositos = await s.Get_DepotsMTYAsync(v_cadena[0]);
            ListItem c;

            for (int i = 0; i < ListDepositos.Count; i++)
            {
                if (ListDepositos[i].item != "0" && ListDepositos[i].item != "-1")
                {
                    c = new ListItem();
                    c.Dato = ListDepositos[i].item;
                    c.Info = ListDepositos[i].value;
                    liDeposito.Add(c);
                }
            }
        }

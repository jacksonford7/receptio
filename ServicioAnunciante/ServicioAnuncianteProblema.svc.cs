...
namespace RECEPTIO.CapaServicioDistribuidos.ServicioAnunciante
{
    public class ServicioAnuncianteProblema : IServicioAnuncianteProblema
    {
        ...
        public void AnunciarProblemaTransaccionPendiente(long idPreGate, short idZona)
        {
            IProblema administradorProblema = null;
            try
            {
                var ctx = new XmlApplicationContext("~/Springs/SpringProblema.xml");
                administradorProblema = (IProblema)ctx["AdministradorProblema"];
                var resultado = administradorProblema.AnunciarProblemaTransaccionPendiente(idPreGate, idZona);
                if (resultado.Item1)
                    EnviarMensajeProblema(resultado);
            }
            catch (Exception)
        ...
        public void AnunciarProblemaServicioWebTransaction(string error, short idZona, int idAplicacion)
        {
            ...
            try
            {
                var ctx = new XmlApplicationContext("~/Springs/SpringProblema.xml");
                administradorProblema = (IProblema)ctx["AdministradorProblema"];
                var resultado = administradorProblema.RegistrarProblemaServicioWebTransaction(error, idZona, idAplicacion);
                if (resultado.Item1)
                    EnviarMensajeProblema(resultado);
            }
            catch (Exception)
